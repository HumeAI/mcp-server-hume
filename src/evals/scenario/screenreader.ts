...
export const screenreaderScenario = async (descriptions: typeof DESCRIPTIONS): Promise<EvalScenario> => {
  const blogParagraphs = (await fs.readFile(path.join(__dirname, '/../data/metasyntactic_blog_post.txt'), 'utf-8')).split('\n\n');
  ...
  return {
    roleplay: {
      ...
      tools: {
        ...
        get_blog_post: getContent('This tool is able to retrieve sections of the blog post at https://metasyntactic-institute.org/blog/guide', {
      ...
      initialMessage: "yo, can you read me the blog post at https://metasyntactic-institute.org/blog/guide",
